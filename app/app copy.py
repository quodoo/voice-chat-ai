...
def sanitize_response(response):
    """
    Clean up model responses before sending to TTS.

    Important: **do not strip non‑ASCII characters** (e.g. Japanese, emojis),
    otherwise UTF‑8 text coming from Ollama will be mangled and Kokoro/OpenAI TTS
    will receive empty or corrupted content.
    """
    # Remove <think>...</think> blocks first (Claude-style thinking tags)
    response = re.sub(r'<think>[\s\S]*?<\/think>', '', response)

    # Strip simple markdown formatting markers but keep the inner text and
    # all Unicode characters intact.
    # - Remove paired asterisks around text: **bold** or *italic*
    response = re.sub(r'\*([^\*]+)\*', r'\1', response)

    # Optionally collapse excessive whitespace
...
def chatgpt_streamed(user_input, system_message, mood_prompt, conversation_history):
    ...
    if MODEL_PROVIDER == 'ollama':
        ...
        try:
            ...
            # Use iter_lines with decode_unicode=True for proper UTF-8 handling
                ...
                try:
                    ...
                    if delta_content:
                        ...
                        while '\n' in line_buffer:
                            ...
                            print(f"{NEON_GREEN}{output_line}{RESET_COLOR}")
                            
                except (json.JSONDecodeError, UnicodeDecodeError) as e:
                    # Debug problematic lines
                    print(f"DEBUG: Error: {e}, line: {repr(line[:100])}")
                    continue
                    
            # Print any remaining content
            if line_buffer:
                full_response += line_buffer
                print(f"{NEON_GREEN}{line_buffer}{RESET_COLOR}")
